...
## SQLite For Local Dev, PostgreSQL Verified (P6)

- Decision: plain local `uvicorn` runs still default to SQLite (`app/core/config.py`'s `database_url` default) for zero-friction local iteration. `docker-compose up` defaults to PostgreSQL 16, with both Alembic migrations and a focused integration/smoke test suite (`tests/test_postgres_migration.py`, `tests/test_postgres_smoke.py`) run against a real PostgreSQL 16 service container in CI.
- Rationale: the query/service layer was already dialect-neutral by construction (no raw SQL, no SQLite-only functions, Python-side time bucketing) -- P6 closed the two real gaps that were dialect-dependent rather than requiring an architectural change:
  - `created_at`/`received_at` columns used `default=func.now()`, a server-side SQL default. SQLite's `now()` compiles to `CURRENT_TIMESTAMP` (always UTC); PostgreSQL's `now()` reflects the session's configured timezone, not guaranteed UTC. Replaced with a Python-side `utcnow()` default (`app/db/base.py`) evaluated identically regardless of backend or server timezone.
  - Alembic's `downgrade()` for tables with `sa.Enum` columns (`event_type`, `session_status`, `zone.type`, `correlation.status`, `role`) dropped the table but not PostgreSQL's underlying native `CREATE TYPE ... AS ENUM`, breaking a downgrade-then-upgrade cycle. Both migrations now explicitly drop their enum types in `downgrade()`.
- Tradeoff: the existing SQLite-backed unit/integration test suite is intentionally left as-is rather than duplicated per-backend -- migrations and cross-cutting flows are what actually differ by dialect, and those are what the new PostgreSQL-specific tests cover.

## Dependency Footprint: Runtime Split From Video/Dev Tooling

- Decision: `requirements.txt` now lists only what the API/dashboard runtime (`app/`) actually imports (FastAPI, SQLAlchemy, Alembic, psycopg, Pydantic, PyJWT, bcrypt, structlog) -- this is what `Dockerfile` installs. `requirements-video.txt` (`-r requirements.txt` plus ultralytics/opencv-python/numpy) and `requirements-dev.txt` (`-r requirements.txt` plus ruff/pytest/httpx) are separate, additive files for local use; neither is installed in the Docker image. `.dockerignore` was added so the build context/image also stop carrying `.git`, `.venv`, `tests/`, `docs/`, and other non-runtime paths (explicitly *not* excluding `data/` or `dashboard/`, both of which are read at runtime -- see their comments in `.dockerignore`).
- Rationale: `app/` has zero imports of `cv2`, `numpy`, `torch`, or `ultralytics` anywhere -- only `pipeline/video/detectors.py` and `pipeline/video/tracking.py` use them, both as lazy in-function imports, and both are invoked only via the standalone `python -m pipeline.video.process_videos` CLI (see README's "Demo Walkthrough"), never from `app/main.py`, any API route, or `docker-compose.yml`. Before this split, `requirements.txt` installed `ultralytics`'s full transitive dependency tree unconditionally -- including the CUDA-enabled `torch` build (~2.7GB of NVIDIA CUDA/cuDNN/cuBLAS/NCCL/Triton packages) even though nothing in the codebase ever requests a GPU device (no `device=`, `.cuda()`, or `.to('cuda')` anywhere) and `docker-compose.yml` configures no GPU passthrough -- so none of it was reachable in this deployment regardless. This made the `api`/`migrate` Docker image 3.11GB for a service that uses none of it. Splitting the requirements files removes ~2.8GB of it from the image with zero behavioral change to the API/dashboard, and zero change to how the video pipeline is installed/run locally (same `python -m pipeline.video.process_videos` command, now preceded by `pip install -r requirements-video.txt`).
- Tradeoff: local dev now has three requirements files instead of one, and `pip install -r requirements.txt` alone (the old one-file command) no longer gives a fully-featured local dev environment -- `requirements-dev.txt` and/or `requirements-video.txt` are needed depending on what you're doing (see README's Quickstart). Dependency version pinning/reproducible-build tooling (a lockfile, or pinned versions in these files) was deliberately left out of this change -- a separate, future reproducibility phase, not entangled with this size/scope split.
...
## P8: Spatial Intelligence Stays Zone-Level, Not Point-Projected

- Decision: `SpatialIntelligenceService` (`app/services/spatial_intelligence_service.py`) visualizes activity by shading each `Zone`'s existing `map_polygon_json` with a real, unchanged `AnalyticsService.zone_dwell_metrics` aggregate. It does not attempt to place an individual event/visitor at a continuous position on the map.
- Rationale: `Event.hotspot_x`/`hotspot_y` and `CameraCoverage`'s polygon are camera-frame-normalized (per camera); `Zone.map_polygon_json` is a separately-drawn, map-image-normalized outline (see "P7: Camera-To-Map Geometry Stays Deliberately Simple" above) -- there is no transform between the two anywhere in this codebase, and the two demo stores' apparent visual agreement is a coincidence of `pipeline.migrate_legacy_store_config` reusing the same hardcoded rectangle for both, not a real correspondence (it already breaks for ST1001, which has two cameras covering one zone). `zone_id` membership and the existing zone-dwell aggregate, by contrast, are already correct and require no new coordinate math. See docs/DESIGN.md's "Zone-Level Spatial Intelligence, Not Point Projection (P8)" section for the full reasoning, including a correction to that document's previously-inaccurate claim that footpoints are projected into layout coordinates.
- Tradeoff: the visualization is coarser than a per-point heatmap would be if one were trustworthy -- a store with only one or two configured zones (as real CCTV-derived data currently has, see below) gets a correspondingly coarse map. Accepted rather than fabricating apparent precision the underlying data/geometry doesn't support. `HeatmapService`'s existing point-based `/heatmap` endpoint is left unchanged (additive, not replacing) -- its camera-frame/map-frame conflation is now documented as a known limitation rather than fixed in this phase.
- Related, separate limitation confirmed while building this: neither ST1001 nor ST1002 (the only stores with real, challenge-video-derived event data) has an uploaded `Map` image in this repository/environment -- `pipeline/migrate_legacy_store_config.py`'s legacy layout image paths have never resolved to a real file here. `tests/test_spatial_intelligence_validation.py` validates zone-level activity aggregation against the real dataset and validates the full map+zone-shading experience separately, against the existing synthetic multi-zone demo dataset -- it does not fabricate a map for the real-data store to force a visual demo.
